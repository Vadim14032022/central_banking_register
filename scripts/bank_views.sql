...
CREATE VIEW cbr.passport_masked AS
SELECT
...
CREATE VIEW cbr.user_masked_contacts AS
SELECT
    u.user_id,
...
FROM
    cbr.user u
JOIN
    cbr.passport p ON u.passport_id = p.passport_id;
...
CREATE VIEW cbr.account_types_with_bank_clean AS
SELECT
    b.bank_nm,
    act.acc_type_txt,
    act.start_dt,
...
FROM
    cbr.user u
LEFT JOIN
    cbr.registry r ON u.user_id = r.user_id
GROUP BY
    u.user_id, u.phone_num;

-- Представление для получения сводной таблицы с информацией о банке, типе счета и балансе
CREATE VIEW cbr.account_summary AS
SELECT
    u.user_id,
    u.phone_num,
    a.account_id,
...
INNER JOIN
    cbr.registry r ON u.user_id = r.user_id
INNER JOIN
    cbr.account a ON r.account_id = a.account_id
INNER JOIN
    cbr.account_type at ON a.acc_type_id = at.acc_type_id;

-- Представление для получения статистики счетов по банкам
CREATE VIEW cbr.account_by_bank_summary AS
SELECT
    b.bank_nm,
...
FROM
    cbr.bank b
JOIN
    cbr.account_type at ON b.bank_id = at.bank_id
JOIN
    cbr.account a ON at.acc_type_id = a.acc_type_id
GROUP BY
    b.bank_nm;

-- Представление для общей статистики пользователей
CREATE VIEW cbr.total_cbr_statistics AS
SELECT
