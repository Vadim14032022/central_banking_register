...
FROM
    cbr.user
WHERE
    email_txt = 'ivanovvasiliy@gmail.com';
...
DELETE FROM cbr.user
WHERE
    email_txt = 'ivanovvasiliy@gmail.com';

-- 6) Удаление информации о паспорте (УДАЛЕНИЕ)
DELETE FROM cbr.passport
WHERE
    passport_no = '937456';
...
FROM
    cbr.passport
WHERE
    cbr.passport.birth_dt > '1980-01-01';
...
SELECT
    at.acc_type_id,
    at.acc_type_txt,
    AVG(a.cur_balance_amt::NUMERIC) AS avg_balance
FROM
    cbr.account a
