...
-- 3) Запрос на добавление колонки с ранжированием 
...
SELECT *,
       ...
       COUNT(*) OVER(PARTITION BY acc_type_id)
FROM (
    SELECT *,
...
-- 4) Запрос на добавление колонки с суммарным балансом 
...
-- 5) Запрос на вывод самого высокого баланса счета для каждого типа счета:
SELECT *,
       MAX(cur_balance_amt) OVER(PARTITION BY acc_type_id) AS max_balance
FROM cbr.account;

-- 6) Запрос на вывод номера телефона клиента с самым длинным списком 
